...
class BitBayAPI(CryptoAPI):
    ...
    def find_best_sell_offer(self, crypto_curr, base_curr, quantity):
        ...
        super(BitBayAPI, self).quick_sort_orderbook_by_rate(orderbook_buy)
        result = []
        ...
        while quantity > 0 and i < len(orderbook_buy):
            ...
            if quantity - buy_offer_quantity >= 0:
                map = {
                    'Quantity': orderbook_buy[i][self.QUANTITY],
                    'Rate': orderbook_buy[i][self.RATE]
                }
                result.append(map)
    ...
    def get_orderbook_sorted(self, crypto, base_curr, type):
        ...
        return super(BitBayAPI, self).quick_sort_orderbook_by_rate(orderbook)
