...
apiUrl2 = 'https://api.bittrex.com/v3'
...
def getOffer(marketSymbol: Tuple[str, str], numberOfOffers: int, apiUrl: str):
    ...
    if apiUrl.__contains__('bitbay'):
        tradeOffers = loadDataFromApi(f'{apiUrl1}/{marketSymbol[0]}{marketSymbol[1]}/orderbook.json')
    else:
        tradeOffers = loadDataFromApi(f'{apiUrl2}/markets/{marketSymbol[0]}-{marketSymbol[1]}/orderbook')

    if tradeOffers:
        bids = tradeOffers["bids" if apiUrl.__contains__('bitbay') else "bid"][:numberOfOffers]
        asks = tradeOffers["asks" if apiUrl.__contains__('bitbay') else "ask"][:numberOfOffers]
        return [bids, asks]
    else:
...
def main():
    ...
    showOffers(marketSymbols, 2, apiUrl1)
