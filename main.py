...
def calculateDifference(orderFrom, orderTo, apiFrom, apiTo, symbol):
    quantity = min(orderFrom['quantity'], orderTo['quantity']) - apiFrom.withdrawalFee(symbol)

    return {
        'rate': (orderFrom['rate'] - orderTo['rate']) * (1 - apiTo.transactionFee),
        'quantity': quantity
    }
...
def calculateProfit(apiFrom, apiTo, symbol):
    ...
    while (profit == None or profit > 0) and len(queueFrom) > 0 and len(queueTo) > 0:
        ...
        difference = calculateDifference(
            orderFrom, orderTo, apiFrom, apiTo, symbol.split('-')[0])
        if profit == None:
        ...
        if orderFrom['quantity'] > orderTo['quantity']:
            queueFrom.append({
                'quantity': orderFrom['quantity'] - orderTo['quantity'],
                'rate': orderFrom['rate']
            })
        elif orderFrom['quantity'] < orderTo['quantity']:
            queueTo.append({
                'quantity': orderTo['quantity'] - orderFrom['quantity'],
                'rate': orderTo['rate']
            })

    if rateSum != 0 and previousProfit < 0 and quantity < 0:
...
def main():
    ...
    # Ex2
    print("Ex2: ")
    print(f"BTC-USD: {calculateProfit(APIS['BITBAY'], APIS['BITTREX'], 'BTC-USD'):.4f}%")
    print(f"ETH-USD: {calculateProfit(APIS['BITBAY'], APIS['BITTREX'], 'ETH-USD'):.4f}%")
    print(f"LTC-USD: {calculateProfit(APIS['BITBAY'], APIS['BITTREX'], 'LTC-USD'):.4f}%")
