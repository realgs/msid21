...
def get_arbitrage(arbitrage_code, currencies, response_bitbay, response_bittrex, buy_offer_number,
    ...
    payment = get_payment(buy_rate, transaction_quantity, FEES[fee_source][TAKER],
                          with_fees)
    profit = get_profit(sell_rate, transaction_quantity, FEES[fee_source_2][TAKER],
...
def get_markets():
    ...
    if response_bittrex is not None and response_bitbay is not None:
        ...
        return markets_list_bittrex, markets_list_bitbay
    else:
        return None, None


def get_common_markets(markets_list_bittrex=None, markets_list_bitbay=None):
    if markets_list_bittrex is None and markets_list_bitbay is None:
        markets_list_bittrex, markets_list_bitbay = get_markets()
    if markets_list_bittrex is not None and markets_list_bitbay is not None:
...
def main():
    ...
    print_sorted_arbitrage_for_markets(get_common_markets(), 'percentage earning', False, True)
    print_sorted_arbitrage_for_markets(get_common_markets(), 'percentage earning', True, False)
